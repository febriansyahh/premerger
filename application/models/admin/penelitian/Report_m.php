...
class Report_m extends CI_Model
{
    ...
    {
        return $this->db->query("SELECT a.*, b.`skim_name`, c.`anggota_nama`, c.`anggota_nidn` FROM `lit_usulan` a LEFT JOIN `lit_skim` b ON a.`skim_id`=b.`skim_id` LEFT JOIN `lit_anggota` c ON c.`usulan_id`=a.`usulan_id` WHERE c.`anggota_posisi` ='ketua' ORDER BY a.`usulan_id` DESC")->result();
    }
